...
func NewClient(options ClientOptions) (*Client, error) {
	...
	case "cubic", "new_reno", "bbr", "bbr_meta_v1", "bbr_quiche", "bbr2", "bbr2_aggressive":
	default:
...
func (c *Client) sendFakePackets(ctx context.Context, conn net.Conn) {
	packets := make([][]byte, 0, 2)
	if len(c.fakeHeartbeatData) > 0 {
		packets = append(packets, c.fakeHeartbeatData)
	}
	if len(c.fakeGameData) > 0 {
		packets = append(packets, c.fakeGameData)
	}
	if len(packets) == 0 {
		return
	}
	deadline := time.Now().Add(c.fakePacketDuration)
	...
	for {
		...
		case t := <-ticker.C:
			...
			conn.Write(packets[index%len(packets)])
			index++
		}
	}
}
...
func (c *Client) offerNew(ctx context.Context) (*clientQUICConnection, error) {
	...
	if c.fakePacketDuration > 0 {
		c.sendFakePackets(ctx, udpConn)
		if ctx.Err() != nil {
			udpConn.Close()
			return nil, ctx.Err()
		}
